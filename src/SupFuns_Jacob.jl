...
vett_funz = zeros(Probl.problem_size,coef_end)
    ...
    else # we have this "else" only in Jacob
        vett_funz[:, 1:funz_end-funz_beg+1] = fy[:,funz_beg:funz_end]
    end
    vett_funz_fft = fft(vett_funz,2)
...
for n in nxi:min(N,nxf)
    ...
    Φ_n = St +
        METH.hα2.*(METH.a0[1:Probl.β_length,n+1].*fy[:,1] .+ zn[:,n+1] .+ Φ)
    yn0 = y[:,n] ; fn0 = f_value(Probl.f_fun(t[n+1], yn0, Probl.param...),Probl.problem_size)
    Jfn0 = Probl.JF(t[n+1],yn0,Probl.param...)
    Gn0 = yn0 - METH.hα2.*fn0 - Φ_n
    stop = zeros(1); stop = false; mu_it = 0
