function [t, y] = fde_pi2_im(alpha,f_fun,J_fun,t0,T,y0,h,param,tol,itmax)
% FDE_PI2_Im  Solves on the interval [t0,T] an initial value problem for a 
...
%             product-integration rule of trapezoidal type with convergence 
%             order equal to 2 (under suitable assumptions of regularity of 
%             the exact solution).
%
...
% Possible usages
% -------------------------------------------------------------------------
% [t, y] = FDE_PI2_Im(alpha,f_fun,J_fun,t0,T,y0,h)
% [t, y] = FDE_PI2_Im(alpha,f_fun,J_fun,t0,T,y0,h,param)
% [t, y] = FDE_PI2_Im(alpha,f_fun,J_fun,t0,T,y0,h,param,tol)
...
nvett = 0 : NNr+1 ; 
an = zeros(Probl.alpha_length,NNr+1) ; a0 = an ;
...
PI.an = an ; PI.a0 = a0 ;
PI.halpha2 = h.^alpha./gamma(alpha+2) ;
...
if Qr >= 0
    ...
    an_fft = zeros(Probl.alpha_length,index_fft(2,Qr+1)) ; 
    for l = 1 : Qr+1 % log2(NNr/r)
        coef_end = 2^l*r ;
        for i_alpha = 1 : Probl.alpha_length
            find_alpha = find(alpha(i_alpha)==alpha(1:i_alpha-1)) ;
            if ~isempty(find_alpha)
                ...
                an_fft(i_alpha,index_fft(1,l):index_fft(2,l)) = fft(PI.an(i_alpha,1:coef_end),coef_end) ;
            end
        end
    end
    PI.an_fft = an_fft ; PI.index_fft = index_fft ;
end
...
Nnxf = min(N,nxf) ; 
    ...
    for i = 1 : Probl.problem_size
        ...
        Z = PI.an_fft(i_alpha,PI.index_fft(1,i_fft):PI.index_fft(2,i_fft)).*vett_funz_fft(i,:) ;
...
for n = nxi : min(N,nxf)
    ...
    for j = nxi : n-1
        Phi = Phi + PI.an(1:Probl.alpha_length,n-j+1).*fy(:,j+1) ;
    end
    %size(St) , size(PI.halpha2) , size(PI.a0(1:Probl.alpha_length,n+1)) , size(fy(:,1))
    Phi_n = St + PI.halpha2.*(PI.a0(1:Probl.alpha_length,n+1).*fy(:,1) + zn(:,n+1) + Phi) ;
        
    yn0 = y(:,n) ; fn0 = f_vectorfield(t(n+1),yn0,Probl) ;
    Jfn0 = Jf_vectorfield(t(n+1),yn0,Probl) ;
    Gn0 = yn0 - PI.halpha2.*fn0 - Phi_n ;
    stop = 0 ; it = 0 ;
    
    while ~stop

        JGn0 = eye(Probl.problem_size,Probl.problem_size) - diag(PI.halpha2)*Jfn0 ;
        yn1 = yn0 - JGn0\Gn0 ;
        fn1 = f_vectorfield(t(n+1),yn1,Probl) ;
        Gn1 = yn1 - PI.halpha2.*fn1 - Phi_n ;
        it = it + 1 ;
