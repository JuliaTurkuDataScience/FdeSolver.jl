function [t, y] = MT_FDE_PI2_Im(al,lam,f_fun,J_fun,t0,T,y0,h,param,tol,itmax)
% -------------------------------------------------------------------------
% MT_FDE_PI2_Im  Solves on the interval [t0,T] an initial value problem
...
%                product-integration rule of trapezoidal type having order 
%                of convergence greater than 1. 
%
...
% Possible usage
% -------------------------------------------------------------------------
% [t, y] = MT_FDE_PI2_Im(al,lam,f_fun,J_fun,t0,T,y0,h)
% [t, y] = MT_FDE_PI2_Im(al,lam,f_fun,J_fun,t0,T,y0,h,param)
% [t, y] = MT_FDE_PI2_Im(al,lam,f_fun,J_fun,t0,T,y0,h,param,tol)
% [t, y] = MT_FDE_PI2_Im(al,lam,f_fun,J_fun,t0,T,y0,h,param,tol,itmax)
%
% -------------------------------------------------------------------------
...
METH.an = an ; METH.a0 = a0 ; METH.C = C ;
...
for n = nxi : min(N,nxf)
    ...
    Gn0 = (1+METH.C)*yn0 - METH.an(Q,1)/Probl.lam_Q*fn0 - Phi_n ;
    stop = 0 ; it = 0 ;
    
    while ~stop
        
        JGn0 = (1+METH.C)*eye(Probl.problem_size,Probl.problem_size) - ...
            METH.an(Q,1)/Probl.lam_Q*Jfn0 ;
        yn1 = yn0 - JGn0\Gn0 ;
        fn1 = f_vectorfield(t(n+1),yn1,Probl) ;
        Gn1 = (1+METH.C)*yn1 - METH.an(Q,1)/Probl.lam_Q*fn1 - Phi_n ;
        it = it + 1 ;
