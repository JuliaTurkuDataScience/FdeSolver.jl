function [t, y] = fde_pi12_pcIM2(alpha,f_fun,J_fun,t0,T,y0,h,param,mu,mu_tol)
...
%                 bn_fft(i_alpha,index_fft(1,l):index_fft(2,l)) = fft(METH.bn(i_alpha,1:coef_end),coef_end) ;
    ...
    METH.an_fft = an_fft ; METH.index_fft = index_fft ;
end
...
%     Phi_n = St + METH.halpha1.*(zn_pred(:,n+1) + Phi) ;
    Phi_n = St + ...
            METH.halpha2.*(METH.a0(1:Probl.alpha_length,n+1).*fy(:,1) + zn_pred(:,n+1) + Phi) ;
    
    yn0 = y(:,n) ; fn0 = f_vectorfield(t(n+1),yn0,Probl) ; % fn0   
    Jfn0 = Jf_vectorfield(t(n+1),yn0,Probl) ;
    Gn0 = fn0 - METH.halpha2.*fn0 - Phi_n ;
